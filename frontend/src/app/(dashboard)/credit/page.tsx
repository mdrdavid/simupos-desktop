...
import { CreditEntry, CreditStatus } from "@/src/types/credit"

export default function CreditListPage() {
  const router = useRouter()
  const { getCreditEntries, loading } = useCredit()
  ...
  const loadData = useCallback(
    async (
      ...
      try {
        const response = await getCreditEntries({
          page,
          limit,
          customerName: search,
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    loadData({ page: 1 }, e.target.value, statusFilter)
  }

  const handleStatusChange = (status: CreditStatus | "all") => {
    setStatusFilter(status)
    loadData({ page: 1 }, searchTerm, status)
  }
      ...
      <Card>
        <CardContent className="p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Button
              variant="outline"
              onClick={() => router.push("/credit/outstanding")}
            >
              Outstanding
            </Button>
          </div>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Tabs
              value={statusFilter}
              onValueChange={(value) =>
                handleStatusChange(value as CreditStatus | "all")
              }
