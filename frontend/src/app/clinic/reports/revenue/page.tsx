...
import { ArrowLeft, Download, TrendingUp, DollarSign, CreditCard, Banknote } from "lucide-react"
...
export default function RevenueReportsPage() {
  ...
  const revenueReport = generateReport("revenue", selectedPeriod, dateRange?.from, dateRange?.to)
  ...
  const dailyRevenueData = eachDayOfInterval({
    ...
    const dayVisits = visits.filter((visit) => {
      ...
      return visitDate.toDateString() === date.toDateString() && visit.status === "Completed"
    })
  ...
  const paymentMethodData = Object.entries(revenueData.paymentMethods || {}).map(([method, amount]) => ({
    name: method,
    value: amount as number,
    percentage: (((amount as number) / revenueData.totalRevenue) * 100).toFixed(1),
  }))
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-600">Financial performance and payment analysis</p>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xl font-bold text-green-600">UGX {revenueData.totalRevenue.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xl font-bold text-blue-600">{revenueData.totalVisits}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            {paymentMethodData.map((method, index) => (
              <div key={method.name} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <p className="font-semibold">UGX {method.value.toLocaleString()}</p>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-green-50 rounded-lg">
              ...
              <p className="text-sm text-green-700">
                Highest daily revenue: UGX {Math.max(...dailyRevenueData.map((d) => d.revenue)).toLocaleString()}
              </p>
            </div>
            ...
            <div className="p-4 bg-blue-50 rounded-lg">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <h4 className="font-semibold text-blue-800">Most Popular Payment</h4>
              </div>
              <p className="text-sm text-blue-700">
                {paymentMethodData.length > 0 ? paymentMethodData[0].name : "N/A"} -{" "}
                {paymentMethodData.length > 0 ? paymentMethodData[0].percentage : 0}% of payments
              </p>
            </div>
          </div>
          ...
          <div className="p-4 bg-yellow-50 rounded-lg">
            ...
            <ul className="text-sm text-yellow-700 space-y-1">
              <li>• Consider promoting digital payment methods to reduce cash handling</li>
              <li>• Monitor daily revenue trends to identify peak service hours</li>
