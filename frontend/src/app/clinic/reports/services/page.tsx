...
import { ArrowLeft, Download, Stethoscope, TrendingUp, Users, DollarSign } from "lucide-react"
...
export default function ServiceReportsPage() {
  ...
  const serviceReport = generateReport("services", selectedPeriod, dateRange?.from, dateRange?.to)
  ...
  const topServicesChartData = serviceData.slice(0, 10).map((item:any) => ({
    name: item.service.name.length > 15 ? item.service.name.substring(0, 15) + "..." : item.service.name,
  ...
  const totalServiceRevenue = serviceData.reduce((sum:any, item:any) => sum + item.revenue, 0)
  const totalServiceCount = serviceData.reduce((sum:any, item:any) => sum + item.count, 0)
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-600">Service performance and popularity analysis</p>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xl font-bold text-green-600">UGX {totalServiceRevenue.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xl font-bold text-blue-600">{totalServiceCount}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xl font-bold text-purple-600">{services.filter((s) => s.isActive).length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <div key={category.name} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{category.percentage}% of services</p>
                  </div>
                </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {serviceData.map((item:any, index:any) => (
              <div key={item.service.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-4 mt-1">
                      ...
                      <span className="text-sm text-gray-600">UGX {item.service.fee.toLocaleString()} per service</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-green-600">UGX {item.revenue.toLocaleString()}</p>
                  <p className="text-sm text-gray-600">{item.count} times provided</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-green-50 rounded-lg">
              ...
              <p className="text-sm text-green-700">
                {serviceData.length > 0 ? serviceData[0].service.name : "N/A"} - UGX{" "}
                {serviceData.length > 0 ? serviceData[0].revenue.toLocaleString() : 0} revenue
              </p>
            </div>
            ...
            <div className="p-4 bg-blue-50 rounded-lg">
              ...
              <p className="text-sm text-blue-700">
                {serviceData.length > 0 ? serviceData.sort((a:any, b:any) => b.count - a.count)[0]?.service.name : "N/A"} -
                {serviceData.length > 0 ? serviceData.sort((a:any, b:any) => b.count - a.count)[0]?.count : 0} times
              </p>
            </div>
          </div>
