...
export default function EditServicePage() {
  ...
  const { serviceTypes, fetchServiceTypes, updateServiceType, loading } =
    useWashingBay();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateServiceType(serviceId, updateData);
      router.push(`/washing-bay/services/${serviceId}`);
    } catch (error: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => router.push(`/washing-bay/services/${serviceId}`)}
          className="h-9 w-9"
        >
