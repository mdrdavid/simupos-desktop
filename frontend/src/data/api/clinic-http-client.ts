export const clinicHttpClient = async (url: string, options?: RequestInit) => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API}/clinic${url}`, options)
  const contentType = response.headers.get("Content-Type")
  const isJson = contentType && contentType.includes("application/json")
  const parseErrorBody = async () => (isJson ? await response.json() : { message: await response.text() })
